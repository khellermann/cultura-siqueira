...
function Inscricoes() {
  ...
  const selectedFields = useMemo(() => {
    ...
    if (selectedOpportunity.type === "edital" && selectedOpportunity.fields) {
      return selectedOpportunity.fields;
    }
    return selectedOpportunity.fields?.length
      ? selectedOpportunity.fields
      : getDefaultRegistrationFields(selectedOpportunity.type);
  }, [selectedOpportunity]);
    ...
    async function loadOpportunities() {
      ...
      try {
        ...
        const activeOpportunities = snapshot.docs
          ...
          .filter(
            (opportunity) =>
              isRegistrationOpportunityOpen(opportunity) &&
              (opportunity.type !== "edital" || Boolean(opportunity.fields?.length)),
          );
          ...
          <div className="mx-auto grid max-w-7xl gap-8 px-6 py-12 md:px-10 md:py-16 lg:grid-cols-12">
            ...
            <div className="lg:col-span-7">
              <form
                ...
                className="border-2 border-[#E2E2EA] bg-white p-6 md:p-8"
              >
                  ...
                  <label className="grid gap-2 text-sm font-semibold text-[#414296]">
                    ...
                    >
                      {opportunities.length === 0 && (
                        <option value="">Nenhuma atividade aberta</option>
                      )}
                      {opportunities.map((opportunity) => (
